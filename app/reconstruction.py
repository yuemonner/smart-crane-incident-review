...
def _bounded_interpretation(evidence_state: dict[str, list[str]], peers: PeerContextResult | dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "why": [
            "Application-version and profile evidence precede the module-health trigger.",
        ...
        "what_contradicts_this": [
            *evidence_state["counterevidence"],
            ...
            "Cellular-only connectivity is not unique to affected cranes.",
            "No matching full module-health failure has been recorded on peer assets.",
        ],
        "what_would_reduce_uncertainty": [
            "Compare firmware and network profile across affected and unaffected cranes.",
            "Retrieve customer-side firewall or network state.",
            "Recover local controller state during the disconnect window.",
            "Compare the same signal against earlier application and profile combinations.",
        ],
...
def _frozen_decision(records: list[CanonicalEvidence], incident_time: datetime, knowledge_time: datetime) -> dict[str, Any]:
    ...
    return {
        ...
        "unknown": [
            ...
            "whether application-version / profile changes affected reconnect behavior",
        ],
...
def _synthetic_similar_episodes(signature: ContextSignature) -> list[SimilarEpisode]:
    rows = [
        ...
        ("episode-crane-24-202607071155", "crane-24", 3, "Field inspection", "hardware-related", False, "same unhealthy module signal, different application version"),
