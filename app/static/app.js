...
function renderUnexpectedShell(){
  ...
  document.querySelector('.decision-under-review').innerHTML='<b>Decision under review:</b> Continue the monitoring deployment or roll back rev 4.9 / config C17?';
  ...
  document.querySelector('.memory').innerHTML='<span class="eyebrow">CURRENT HUMAN DECISION</span><p><b>No human decision recorded yet.</b></p><p>When the team records a decision, Veyra freezes the evidence available at that time. Later evidence updates the current view, not the historical decision.</p><div class="memory-grid"><input id="memoryOwner" placeholder="Owner"><input id="memoryCheckpoint" type="datetime-local"><select id="memoryDecision"><option value="">Decision not recorded</option><optgroup label="Record decision"><option>Continue investigation</option><option>Hold production deployment</option><option>Roll back</option><option>Redeploy</option></optgroup><optgroup label="Follow-up"><option>Test again</option></optgroup></select><input id="memoryOutcome" placeholder="Observed outcome after the decision"><button id="saveMemory">Save local record</button></div><small id="memoryMessage"></small>';
  bindMemorySave();
  if($('livePlay')){
...
function renderRolloutScenario(){
  ...
  document.querySelector('.decision-under-review').innerHTML='<b>Decision under review:</b> Proceed with image 0.36 rollout to the selected cranes?';
  $('modeBanner').className='mode-banner demo';$('modeBanner').innerHTML='<b>DEMO SCENARIO</b><span>Synthetic pre-deployment rollout review. Veyra does not execute the change.</span>';
  $('incidentSelect').innerHTML='<option>Image 0.36 · software rollout decision · 14/08/2026, 17:05</option>';
  text('freshnessState','SYNTHETIC');text('freshnessDetail','Planned software deployment review');
  document.querySelector('#overview .context-strip').innerHTML='<div><span>Decision time</span><b>17:05 UTC</b></div><div><span>Assets in scope</span><b>18 cranes</b></div><div><span>Package</span><b>4.2 GB</b></div><div><span>Connectivity</span><b>7 cellular-only</b></div><div><span>Recovery gaps</span><b>2 cranes</b></div>';
  document.querySelector('.overview-grid').innerHTML='<article class="panel summary-card"><span class="eyebrow">TRIGGER</span><strong>Rollout</strong><small>planned software deployment review</small></article><article class="panel summary-card"><span class="eyebrow">ASSETS IN SCOPE</span><strong>18</strong><small>selected cranes</small></article><article class="panel summary-card"><span class="eyebrow">BLOCKERS</span><strong>2</strong><small>cranes missing recovery evidence</small></article><article class="panel summary-card"><span class="eyebrow">CURRENT DECISION</span><strong>PENDING</strong><small>No human decision recorded</small></article>';
  document.querySelector('.decision-conditions').innerHTML='<span class="eyebrow">CUSTOMER RULE SET</span><h3>Deployment-review policy v3.2</h3><div class="condition-grid"><div><b>Confirmed</b><span>Target image / build validated</span></div><div><b>Confirmed</b><span>Package integrity / checksum</span></div><div><b>Confirmed</b><span>Last known good image</span></div><div><b>Confirmed</b><span>Previous rollout outcome</span></div><div class="partial"><b>Partial</b><span>Connectivity baseline</span></div><div class="missing"><b>Missing</b><span>Recovery path for 2 cranes</span></div><div class="missing"><b>Missing</b><span>Remote-access path for 2 cranes</span></div></div><small>5 of 7 conditions have supporting evidence across the selected cranes. Two cranes do not currently have evidence of a confirmed recovery path. Veyra does not recommend an action.</small>';
  document.querySelector('.director-grid').innerHTML='<article class="panel lkg-card"><span class="eyebrow">LAST KNOWN GOOD</span><div class="state-pill established">ESTABLISHED FROM EVIDENCE</div><dl><dt>Image</dt><dd>0.35</dd><dt>Rollback package</dt><dd>0.35 available</dd><dt>Validation</dt><dd>Build 2214 passed · checksum available</dd><dt>History</dt><dd>15 cranes have previous rollout outcomes</dd></dl><small>Last known good is source-linked; it does not prove every selected crane has the same recovery path.</small></article><article class="panel scope-card"><span class="eyebrow">DEPLOYMENT CONTEXT</span><div class="state-pill observed">PARTIAL COVERAGE</div><p>Same software change. Different operational context.</p><dl><dt>Stable + recovery confirmed</dt><dd>11 cranes</dd><dt>Cellular-only</dt><dd>7 cranes</dd><dt>Previous interruptions</dt><dd>3 cranes</dd></dl><small>Comparison supports review prioritization; it does not decide whether rollout should proceed.</small></article>';
...
function renderRolloutLive(showOutcome){
  ...
  document.querySelector('#live-reconstruction .live-hero h2').textContent='Before rollout → human decision → external deployment → outcome';
  document.querySelector('#live-reconstruction .lede').textContent='Synthetic Smart Crane rollout evidence. Veyra reconstructs decision context before OTA deployment is executed, then observes the outcome and links it back to the original decision.';
  document.querySelector('#live-reconstruction .context-strip').innerHTML='<div><span>Target image</span><b>0.36</b></div><div><span>Selected cranes</span><b>18</b></div><div><span>Package</span><b>4.2 GB</b></div><div><span>Cellular-only</span><b>7</b></div><div><span>Write access</span><b>None</b></div>';
  $('liveChange').textContent='Image 0.36 selected for rollout review';$('liveReviewStatus').textContent=rolloutDecisionRecorded?'DECISION RECORDED':'DECISION PENDING';$('liveKnowledgeTime').textContent='17:05';
  const steps=['Build 2214 passed validation','Package checksum available','18 selected cranes loaded from device inventory','7 selected cranes are cellular-only','3 previous interrupted transfers found','2 recovery paths not established'];
  ...
  $('liveChanges').innerHTML=[['Current image','0.35','0.36'],['Build','previous production build','Build 2214 passed'],['Package','current image','4.2 GB target package'],['Rollback','image 0.35 installed','image 0.35 rollback available'],['Access context','not evaluated','2 remote/recovery paths not established']].map(([s,b,a])=>`<div><span>${esc(s)}</span><b>${esc(b)} → ${esc(a)}</b><small>synthetic source evidence · sequence only</small></div>`).join('');
  liveList('liveObserved',['target image 0.36 selected','Build 2214 passed','package checksum is available','18 selected cranes are in scope','7 selected cranes are cellular-only']);
  liveList('liveHuman',['team intends to review before deploying to selected cranes','site access constraints should be checked before rollout']);
  liveList('liveInferred',['rollout context differs by crane because connectivity and recovery paths differ','Crane-11 and Crane-12 need additional verification before a clean decision record']);
  liveList('liveNotEstablished',['recovery path for Crane-11','remote-access path for Crane-12','exact connectivity condition during previous interrupted transfers']);
  liveList('liveInterpretation',['The software package is validated and integrity evidence exists.','Most selected cranes have enough supporting context for review.','Two cranes lack recovery/access evidence, so the decision record would contain explicit gaps.']);
  liveList('liveWhy',['Build and checksum evidence support the target package.','Device inventory and connectivity history establish the selected crane set.','Previous rollout outcomes establish which assets have recovered from similar updates before.']);
  ...
  liveList('liveReduce',['verify recovery path for Crane-11','verify remote-access path for Crane-12','capture transfer interruption markers and post-deployment reachability checks']);
  ...
  $('liveGaps').innerHTML=[['COMPLETE','image version','current and target images are source-linked'],['COMPLETE','package checksum','integrity evidence is available'],['PARTIAL','connectivity baseline','7 selected cranes are cellular-only'],['MISSING','recovery path','Crane-11 and Crane-12 need verification'],['MISSING','remote access','Crane-12 path is not established']].map(([s,f,e])=>`<div class="${s.toLowerCase()}"><b>${esc(s)}</b><span>${esc(f)}</span><small>${esc(e)}</small></div>`).join('');
  $('liveDecision').innerHTML=rolloutDecisionRecorded?'<h3>Proceed with 16 selected cranes</h3><p>Hold Crane-11 and Crane-12 pending recovery-path verification.</p><div class="decision-cols"><div><b>Known</b><ul><li>image 0.36 passed validation</li><li>checksum and rollback image are available</li><li>7 selected cranes are cellular-only</li></ul></div><div><b>Unknown / missing</b><ul><li>verified recovery path for Crane-11</li><li>verified remote-access path for Crane-12</li></ul></div></div><small>Decision recorded with evidence available at 17:05 UTC.</small>':'<p>No human decision has been recorded yet.</p><div class="decision-options"><span>Proceed with selected cranes</span><span>Hold selected cranes</span><span>Retry under different connectivity conditions</span><span>Delay rollout</span><span>Cancel rollout</span></div>';
  $('liveLearning').innerHTML=showOutcome?'<h3>Outcome linked to decision</h3><table><tbody><tr><td>Completed normally</td><td>15</td><td>updated cranes remained reachable</td></tr><tr><td>Interrupted and recovered</td><td>1</td><td>cellular transfer resumed</td></tr><tr><td>Held before deployment</td><td>2</td><td>remained on image 0.35</td></tr><tr><td>Unrecoverable deployments</td><td>0</td><td>none recorded</td></tr></tbody></table><p>This rollout outcome becomes context for the next software deployment review.</p><small>Historical outcomes are context, not a recommendation.</small>':'<h3>Historical rollout context</h3><p>4 similar previous rollout reviews.</p><table><tbody><tr><td>Proceed with selected cranes</td><td>2</td><td>successful rollout</td></tr><tr><td>Hold selected cranes</td><td>1</td><td>avoided incomplete recovery context</td></tr><tr><td>Retry under different connectivity conditions</td><td>1</td><td>interrupted but recovered</td></tr></tbody></table><small>Historical outcomes are context, not a recommendation.</small>';
  ...
  $('liveBoundary').textContent='Synthetic/demo data only. Read-only evidence workspace: Veyra reconstructs context before the rollout, the customer OTA system executes the deployment after human approval, and Veyra observes the outcome. No OTA execution, VPN management, source writes, or device control.';
}
function renderRolloutTeamViews(showOutcome){
  ...
  document.querySelector('#where .lede').textContent='Compare selected cranes before deployment is executed and before a human deploy / hold / retry decision is recorded.';
  ...
  setList('confirmed',['18 selected cranes are in scope.','11 have stable connectivity and recovery path confirmed.','image 0.36 build, checksum and rollback image evidence are available.']);
  ...
  document.querySelector('#changed .context-strip').innerHTML='<div><span>Current image</span><b>0.35</b></div><div><span>Target image</span><b>0.36</b></div><div><span>Build</span><b>2214 passed</b></div><div><span>Package</span><b>4.2 GB</b></div><div><span>Checksum</span><b>Available</b></div>';
  $('changes').innerHTML=[['Image','0.35','0.36'],['Build','previous production build','Build 2214 passed'],['Package','current runtime image','4.2 GB package'],['Integrity','not required for current state','checksum available'],['Rollback','image 0.35 installed','image 0.35 rollback package available'],['Evidence sources','single deployment manifest','ADO build · manifest · inventory · connectivity · access policy · rollout history']].map(([l,b,a])=>`<article class="diffrow"><span>${esc(l)}</span><div><small>CURRENT / EVIDENCE</small><b>${esc(b)}</b></div><i>→</i><div><small>TARGET / SUPPORTING CONTEXT</small><b>${esc(a)}</b></div></article>`).join('');
  $('timeline').innerHTML=['Azure DevOps build · Build 2214 passed','Deployment manifest · image 0.36 package selected','Device inventory · 18 selected cranes in scope','Connectivity history · 7 cellular-only','Remote-access configuration · 2 paths not established','Previous rollout outcomes · 3 prior interrupted transfers'].map((x,i)=>`<div class="event supporting observed"><time>SYNTHETIC · SOURCE ${i+1}</time><b>${esc(x)}</b><div class="metric-row"><span>demo data</span><span>read-only</span></div><small class="evidence-meta">OBSERVED · SOURCE REFERENCE · DEMO</small></div>`).join('');
  setList('provenanceLimits',['Synthetic/demo data only.','Decision context only: no OTA execution, no VPN management, no device control.']);
  setList('alreadyReconstructable',['image version','build and test result','package checksum','device connection type','deployment timestamp','previous rollout outcome']);
