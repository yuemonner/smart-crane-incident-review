...
function renderRolloutLive(showOutcome){
  ...
  document.querySelector('#live-reconstruction .lede').textContent='Synthetic Smart Crane rollout evidence. The workspace reconstructs decision context before OTA deployment is executed, then observes the outcome and links it back to the original decision.';
  ...
  $('liveBoundary').textContent='Synthetic/demo data only. Read-only evidence workspace: context is reconstructed before the rollout, the customer OTA system executes the deployment after human approval, and the outcome is observed afterward. No OTA execution, VPN management, source writes, or device control.';
}
