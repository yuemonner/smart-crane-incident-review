...
function clearWorkspace(mode){
  ...
  $('directorEscalation').textContent='Not evaluated';
...
function render(i,w){
  ...
  const escalated=w.customer_count>1&&w.precursor_count>0;$('attentionState').textContent=escalated?'RULE MATCHED':'NO RULE MATCHED';$('directorEscalation').textContent=escalated?'Required under current rules':'Not required under current rules';$('escalationTitle').textContent=escalated?'Explicit review-escalation rule matched':'No review-escalation rule matched';$('escalationReasons').innerHTML=escalated?`<li>Matching telemetry is present on ${w.precursor_count} peer assets.</li><li>Exposed assets span ${w.customer_count} customer fleets.</li><li>Root cause remains not established.</li>`:'<li>No configured threshold is currently met.</li>';
  $('noSignal').textContent=$('noSignalCopy').textContent=Math.max(0,w.exposed_count-w.precursor_count);
  $('matches').innerHTML=w.matches.map(m=>`<div class="match"><div><b>${esc(m.entity_id)}</b><small>${esc(m.customer)} · ${esc(m.matched_factors.join(' · '))}</small></div><span class="tag ${m.precursor_detected?'hot':''}">${m.precursor_detected?'HIGH PRIORITY':'REVIEW'}</span><span class="score">${m.matched_factors.length}/3 signals</span></div>`).join('');
...
function renderKnowledgeState(i,w){
  ...
  setList('ksObserved',[
    alarm?`${alarm.title.replace('; alert delivery under review','')} during operation; alert delivery is under review.`:'Alarm evidence is not present',
...
async function loadMemory(){if(!currentIncident)return;const r=await fetch(`/api/memory/incidents/${encodeURIComponent(currentIncident.id)}`);if(!r.ok){$('recordedDecision').textContent='Pending team decision';$('memoryOutcome').value='';return}const m=await r.json();$('memoryOwner').value=m.owner||'';$('memoryCheckpoint').value=m.checkpoint_at?m.checkpoint_at.slice(0,16):'';$('memoryDecision').value=m.decision||'';$('memoryOutcome').value=m.outcome||'';$('incidentOwner').textContent=m.owner||$('incidentOwner').textContent;$('checkpointDue').textContent=m.checkpoint_at?new Date(m.checkpoint_at).toLocaleString():$('checkpointDue').textContent;$('recordedDecision').textContent=m.decision?(m.outcome?`${m.decision} · outcome: ${m.outcome}`:m.decision):'Pending team decision'}
