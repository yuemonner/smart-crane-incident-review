...
function renderRolloutScenario(){
  ...
  $('modeBanner').className='mode-banner demo';$('modeBanner').innerHTML='<b>DEMO SCENARIO</b><span>Synthetic planned rollout evidence only. No production evidence is shown.</span>';
...
function renderRolloutLive(showOutcome){
  ...
  document.querySelector('#live-reconstruction .live-hero h2').textContent='Planned rollout → evidence review → human decision → outcome';
  document.querySelector('#live-reconstruction .lede').textContent='Synthetic Smart Crane rollout evidence. Veyra reconstructs context before a planned software change; it does not execute deployments or control devices.';
  ...
  $('liveBoundary').textContent='Synthetic/demo data only. Read-only evidence workspace: no OTA execution, no VPN management, no source writes, no device control.';
}
function renderRolloutTeamViews(showOutcome){
  ...
  document.querySelector('#where .lede').textContent='Compare selected cranes before a human deploy / hold / retry decision is recorded.';
