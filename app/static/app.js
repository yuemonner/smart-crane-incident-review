...
function render(i,w){
  ...
  const escalated=w.customer_count>1&&w.precursor_count>0;$('attentionState').textContent=escalated?'RULE MATCHED':'NO RULE MATCHED';$('escalationTitle').textContent=escalated?'Explicit director-attention rule matched':'No director-attention rule matched';$('escalationReasons').innerHTML=escalated?`<li>Matching telemetry is present on ${w.precursor_count} peer assets.</li><li>Exposed assets span ${w.customer_count} customer fleets.</li><li>Root cause remains not established.</li>`:'<li>No configured threshold is currently met.</li>';
