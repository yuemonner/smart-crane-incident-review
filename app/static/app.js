...
const fmtUTC=value=>{const d=new Date(value);return `${String(d.getUTCDate()).padStart(2,'0')} ${months[d.getUTCMonth()]} ${String(d.getUTCHours()).padStart(2,'0')}:${String(d.getUTCMinutes()).padStart(2,'0')} UTC`};
...
async function loadLiveReview(){
  ...
  liveMax=data.max_step;$('liveStep').textContent=`Step ${data.step} / ${data.max_step}`;$('liveKnowledgeTime').textContent=fmtUTC(data.knowledge_time);
  ...
  $('liveEvents').innerHTML=data.event_stream.map(e=>`<div class="stream-event ${e.index===data.step?'active':''}"><time>${fmtUTC(e.event_time)}</time><b>${esc(e.title)}</b><small>${esc(e.source)} · ${esc(e.reconstruction_note)}</small></div>`).join('');
  $('liveChanges').innerHTML=(data.reconstruct_what_changed||[]).slice(0,6).map(c=>`<div><span>${esc(c.subject)}</span><b>${esc(c.before||'—')} → ${esc(c.after||'—')}</b><small>${esc(c.source_system)} · ${fmtUTC(c.event_time)} · ${esc(c.evidence_strength)}</small></div>`).join('')||'<p>No changes reconstructed yet.</p>';
...
async function loadIncidents(){
  ...
  $('incidentSelect').innerHTML=list.length?list.map(x=>`<option value="${esc(x.id)}" data-entity="${esc(x.entity_id)}" data-time="${esc(x.incident_time)}">${esc(x.entity_id)} · ${esc(x.title)} · ${fmtUTC(x.incident_time)}</option>`).join(''):'<option value="">No review triggers in this evidence mode</option>';
...
function renderUnexpectedShell(){
  ...
  document.querySelector('#overview .context-strip').innerHTML='<div><span>Last known healthy</span><b>12 Aug · 05:32 UTC</b></div><div><span>Recent change</span><b>App 0.36 · 13 Aug</b></div><div><span>First abnormal</span><b>14 Aug · 15:32 UTC</b></div><div><span>Human discovery</span><b>21 Aug · 10:14 UTC</b></div><div><span>Detection gap</span><b>6d 18h</b></div>';
...
function renderRolloutScenario(){
  ...
  $('incidentSelect').innerHTML='<option>Monitoring image 0.36 · rollout decision · 14 Aug 17:05 UTC</option>';
  ...
  document.querySelector('.related-memory').innerHTML='<span class="eyebrow">RELATED MEMORY</span><h3>Similar pattern detected with Crane-09</h3><p>Previous rollout: cellular transfer interrupted, then resumed after retry under stronger connection conditions.</p><small>Previous decisions are context for this review, not a recommendation.</small>';
...
function renderRolloutLive(showOutcome){
  ...
  $('liveChange').textContent='Monitoring image 0.36 selected for rollout review';$('liveReviewStatus').textContent=rolloutDecisionRecorded?'DECISION RECORDED':'DECISION PENDING';$('liveKnowledgeTime').textContent='17:05 UTC';
...
function render(i,w){
  ...
  const c=i.coordination||{};text('incidentOwner',c.owner||'Not recorded');text('adoState',c.state||'No linked ADO work item');text('investigationTask',c.work_item_id?`ADO ${c.work_item_id}`:'Not recorded');text('checkpointDue',c.checkpoint_at?fmtUTC(c.checkpoint_at):'Not recorded');
  const l=i.last_known_good||{};$('lkgState').textContent=l.established?'ESTABLISHED FROM EVIDENCE':'NOT ESTABLISHED';$('lkgState').className=`state-pill ${l.established?'established':''}`;$('lkgObserved').textContent=l.observed_at?fmtUTC(l.observed_at):'Not recorded';$('lkgSoftware').textContent=l.software_revision||'Not recorded';$('lkgConfig').textContent=l.config_profile||'Not recorded';$('lkgRuntime').textContent=l.runtime_state||'Not recorded';$('lkgTests').textContent=l.test_evidence?.join(' · ')||'Not recorded';$('lkgValidator').textContent=l.validated_by||'Not recorded';$('lkgLimitation').textContent=l.limitation||'';
  ...
  const escalated=w.customer_count>1&&w.precursor_count>0;$('attentionState').textContent=escalated?'OPEN':'NO PEER SIGNAL';text('directorEscalation',escalated?'Module-health signal + peer matches':'No peer match loaded');$('escalationTitle').textContent=escalated?'Module-health signal detected with matching peer context':'No matching peer signal loaded';$('escalationReasons').innerHTML=escalated?`<li>Review opened because a module-health signal was detected on the review crane.</li><li>Matching precursor signals were found on ${w.precursor_count} peer assets.</li><li>Cause remains not established.</li>`:'<li>No comparable peer signal is currently loaded.</li>';
  ...
  $('matches').innerHTML=topMatches.map(m=>`<div class="match"><div><b>${esc(m.entity_id)}</b><small>${esc(m.customer)} · ${esc(m.matched_factors.join(' · '))}</small></div><span class="tag ${m.precursor_detected?'hot':''}">${m.precursor_detected?'MATCHING SIGNAL':'REVIEW'}</span><span class="score">${m.matched_factors.length}/3 signals</span></div>`).join('')+(remaining?`<div class="match-more">+${remaining} additional exposed assets</div>`:'');
  ...
  const newest=i.freshest_source_at,oldest=i.stalest_source_at;$('freshnessState').textContent=i.evidence_mode.toUpperCase();$('freshnessDetail').textContent=newest?`Retrieved ${fmtUTC(newest)}${oldest&&oldest!==newest?` · oldest retrieval ${fmtUTC(oldest)}`:''}`:'Retrieval timestamp not recorded.';
}
function renderKnowledgeState(i,w){
  ...
  setList('ksObserved',[
    alarm?`${alarm.title.replace(' event with connectivity under review','')} opened a review; module health is under review.`:'Module-health evidence is not present',
    deployment?`${deployment.attributes?.firmware||deployment.attributes?.application||'application'} active in the review window`:'Active application is not established',
    i.status.confirmed.find(x=>x.includes('module'))||'Module-health status requires review',
  ]);
  setList('ksSuspected',[
    'device profile or reconnect behavior may be related',
    'customer-site network rules may be part of the exposure surface',
  ]);
  setList('ksUnknown',[
  ...
  setList('ksCounter',[
    ...
    `${w.matches.filter(m=>!m.precursor_detected).length} reviewed peer records do not establish a matching module-health issue`,
