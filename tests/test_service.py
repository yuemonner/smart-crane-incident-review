...
def test_where_else_expected_demo_counts_and_ranking():
    ...
    assert all(x.precursor_detected for x in result.matches[:11])
    assert all(not x.precursor_detected for x in result.matches[11:])
...
def test_last_known_good_is_explicit_and_traceable():
    ...
    assert lkg.software_revision == "rev 217"
...
def test_device_vs_deployment_scope_does_not_overclaim():
    ...
    assert scope.peer_signal_count == 11
